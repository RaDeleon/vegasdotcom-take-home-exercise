...
const getContentTypeFromExt = ext =>
  simpleContentTypesByExt[ext] || "text/plain";
...
function handleRequest(req, res) {
  ...
  setPathAndStat(fullpath)
    .then(stats => {
      return stats.isDirectory()
        ? // assume that if this is a directory, then it might contain a file named index${defaultExtension}
          setPathAndStat(path.join(fullpath, "index" + defaultExtension)).then(
            stats => fullpath
          )
        : fullpath;
    })
    ...
    .catch(err =>
      setPathAndStat(fullpath + defaultExtension).then(stats => fullpath)
    )
    ...
    .then(r => {
      ...
      const contentType = getContentTypeFromExt(
        isError ? ".json" : path.extname(fullpath)
      );

      if (isError) {
        console.error(`:: ERROR :: <- Unable to read file at ${fullpath}`);
        console.error(
          `:: ERROR :: <- Request for ${uri} returned an error: ${r.body}.`
        );
      } else {
        console.log(`:: INFO :: <- Read file from ${fullpath}.`);
        console.log(
          `:: INFO :: <- Request for ${uri} successfully returned. Content type sent ${contentType}.`
        );
      }
...
`);
  ...
  console.error(
    `:: ERROR :: Unable to read from public directory located at ${publicLocation}`
  );
  console.error(
    `:: INFO :: Try running the app again with a valid public directory (second argument passed to script) `
  );
  process.exitCode = 1;
}
